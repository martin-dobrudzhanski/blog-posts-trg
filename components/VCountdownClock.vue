<template>
  <components :is="tag" v-bind="attrs" v-on="$listeners">
    ...
    <slot
      ...
      name="process"
    />
    <slot v-if="state === 'finised'" v-bind="this._self" name="finish" />
...
export default {
  ...
  methods: {
    startCountdown(restart) {
      ...
      if (!vm.actualEndTime) {
        vm.actualEndTime =
          vm.endTime || new Date().getTime() + (vm.remainingTime || vm.leftTime)
      }
      if (vm.state === 'paused') {
    ...
    doCountdown() {
      ...
      const leftTime = new Date(vm.actualEndTime).getTime() - new Date().getTime()
      if (leftTime > 0) {
        const t = {}
