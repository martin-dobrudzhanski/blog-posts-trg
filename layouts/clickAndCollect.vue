  ...
  >
    ...
    <Headroom :offset="64" :zIndex="9997" :disabled="headerDrawerOpen">
      <MainHeader
        show-basket-cta
        :show-cta="false"
        :show-drawer-toggle="!isClickAndCollectLandingPage"
        :basket-count="clickAndCollectBasketCount"
        :basket-link="headerBasketLink"
        showDrawerToggle
      />
    </Headroom>
    ...
    <BottomNavigation
      v-if="isClickAndCollectLandingPage && (deviceXs || deviceSm)"
    />
...
export default {
  ...
  computed: {
    ...
    headerBasketLink() {
      const restaurantSlug = get(this.clickAndCollectLocation, 'slug', '');

      return `/restaurants/${restaurantSlug}/takeaway/basket`;
    },
    isClickAndCollectLandingPage() {
      return this.$route.name === 'takeaway';
    }
  },
  methods: {
    ...mapActions({
